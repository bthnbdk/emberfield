...
async function launchBrowser() {
  // Allow forcing a specific Chromium build when the bundled revision is
  // unavailable (e.g. distro-managed Playwright on newer Ubuntu).
  const forced = process.env.PLAYWRIGHT_CHROMIUM_PATH;
  if (forced) {
    return chromium.launch({ executablePath: forced });
  }
  try {
