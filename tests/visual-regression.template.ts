...
// supports 'active-play', 'gameover', and 'victory'; add baselines for your
// game's own states (fail/retry, menus, boss phases) as you implement them.
...
test('gameover state visual baseline', async ({ page }, testInfo) => {
  await prepareDeterministicScreenshot(page, 'gameover');
  await expect(page).toHaveScreenshot(`gameover-${testInfo.project.name}.png`, {
    fullPage: true,
    maxDiffPixelRatio: 0.015,
  });
});
